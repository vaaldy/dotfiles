#!/bin/bash

CONFIG_DIR="$HOME/.config/sketchybar"
source "$CONFIG_DIR/colors.sh"

cur_win="$(yabai -m query --windows --window 2>/dev/null)"
if [[ -n "$cur_win" ]]; then
  cur_id="$(jq -r '.id // empty' <<<"$cur_win")"
  cur_space="$(jq -r '.space // empty' <<<"$cur_win")"
  [[ -n "$cur_id" && -n "$cur_space" ]] && echo "$cur_id" > "/tmp/yabai_last_focus_space_${cur_space}"
fi

windows="$(yabai -m query --windows 2>/dev/null)"
[[ -z "$windows" ]] && exit 0

active_map='{}'
for f in /tmp/yabai_last_focus_space_*; do
  [[ -f "$f" ]] || continue
  sp="${f##*_}"
  wid="$(<"$f")"
  [[ -n "$wid" ]] && active_map="$(jq -c --arg sp "$sp" --arg wid "$wid" '. + {($sp): ($wid|tonumber)}' <<<"$active_map")"
done

current_items="$(sketchybar --query bar 2>/dev/null | jq -r '
  [.items[] | select(startswith("app."))] | .[]')"

# Extract standard application windows with per-space active window resolution
desired_data="$(jq -nr \
  --argjson windows "$windows" \
  --argjson active_map "$active_map" \
  '
  ($windows | [.[] | select(."is-minimized" == false and ."is-hidden" == false and (.layer == "normal" or (.app | test("paraview"; "i"))) and (."subrole" == "AXStandardWindow" or (."role" == "AXWindow" and ."can-move" == true) or (.app | test("paraview"; "i"))) and .app != "" and .title != "")]) as $std_wins |
  $std_wins | group_by(.space) | map(
    . as $space_wins |
    ($space_wins[0].space | tostring) as $sp |
    ( ([$space_wins[] | select(."has-focus" == true)][0].id) //
      ($active_map[$sp]) //
      $space_wins[0].id
    ) as $active_id |
    $space_wins | sort_by(.display, .space, .frame.x, .frame.y)[] |
    [ .display, .space, .id, .app, (.id == $active_id) ]
  )[] | @tsv
')"

desired_names=()
args=()

while IFS=$'\t' read -r display space id app is_active; do
  [[ -n "$id" ]] || continue
  name="app.$display.$id"
  desired_names+=("$name")

  if [[ "$is_active" == "true" ]]; then
    color=$PURPLE text=$BG_DARK
  else
    color=0x00000000 text=$MUTED
  fi

  if ! grep -qx "$name" <<<"$current_items"; then
    args+=(--add item "$name" q
           --set "$name" display="$display" space="$space"
             label="$app" icon.drawing=off label.color="$text"
             background.drawing=on background.color="$color"
             background.corner_radius=9 background.height=24
             background.padding_left=3 background.padding_right=3
             label.padding_left=8 label.padding_right=8
             click_script="yabai -m window $id --focus; sketchybar --trigger window_focus"
           --move "$name" before apps_end)
  else
    args+=(--set "$name" space="$space" display="$display"
             label="$app" label.color="$text"
             background.color="$color"
             background.padding_left=3 background.padding_right=3
             label.padding_left=8 label.padding_right=8)
  fi
done <<<"$desired_data"

while IFS= read -r item; do
  [[ -n "$item" ]] || continue
  if ! printf '%s\n' "${desired_names[@]}" | grep -qx "$item"; then
    args+=(--remove "$item")
  fi
done <<<"$current_items"

((${#args[@]} > 0)) && sketchybar "${args[@]}" >/dev/null
